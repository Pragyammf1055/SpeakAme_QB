...
//                    Log.d(TAG, "Get full QBChatDialog 1 :- " + SerializationUtils.deserialize(qbChatDialogBytes));
    ...
    public static ArrayList<String> getDateByGroup(SQLiteDatabase db, String which, String keyname) {
        Log.d(TAG, " getDatafor dd:  -- " + " in methoddd");

        //  SELECT  DBTable.KEY_DATE FROM chatTabel where 1 and ToID = 165 and FromID = 156  group by "messageDate"
        //query = "select * from " + DBTable.TBL_CHAT + " where " + DBTable.KEY_RECEIVER + " = '" + keyname + "' ORDER BY " + DBTable.KEY_ID + " ASC ";
        ArrayList<String> dateFromDataBase = new ArrayList<>();
        try {

//        String query = "select " + DBTable.KEY_DATE + " from " + DBTable.TBL_CHAT + " where  1 and " + DBTable.KEY_RECEIVER + " = '" + keyname + "' " + "group by " + DBTable.KEY_DATE;
            String query = "select " + DBTable.KEY_DATE + " from " + DBTable.TBL_CHAT + " where " + DBTable.KEY_RECEIVER + " = '" + keyname + "' " + "group by " + DBTable.KEY_DATE;
            //  String query = "SELECT "+DBTable.KEY_DATE + " FROM "+DBTable.TBL_CHAT + " GROUP BY "+DBTable.KEY_DATE;
            //  SELECT NAME, SUM(SALARY) FROM COMPANY GROUP BY NAME ORDER BY NAME DESC;
            Log.d(TAG, " getDateByGroup : --- " + query);
            ...
            if (cursor.moveToFirst()) {
                do {
                    // Log.d("CHATLISTSS count 1", cursor.getCount() + "\n" + cursor.getString(cursor.getColumnIndex(DBTable.KEY_SENDER)));
                    String date = cursor.getString(cursor.getColumnIndex(DBTable.KEY_DATE));
                    Log.d(TAG, " dateFromDataBase : -- " + date);
...
//closeConnecion();
    ...
    public static String getResponseFromdataBase(SQLiteDatabase db, Context context) {
//dsvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
// ArrayList<AllBeans> allbeansList = new ArrayList<AllBeans>();
        String response = "";
        try {
            String query;
            query = "select * from " + DBTable.TBL_CONTACTIMPORT + " where " + AppPreferences.LOGINID + " = '" + AppPreferences.getLoginId(context) + "'";
            System.out.println("query GetRespponcs From data Base : " + query);
            Cursor cursor = db.rawQuery(query, null);
            if (cursor == null) {
                return response;
            } else if (cursor.getCount() == 0) {
            ...
            if (cursor.moveToFirst()) {
                ...
                do {
                    response = cursor.getString(cursor.getColumnIndex(DBTable.KEY_RESPONSE));
                } while (cursor.moveToNext());
            }
            if (cursor != null && !cursor.isClosed()) {
                cursor.close();
...
//**************************************************************************************//
    ...
    public static String getUserUpdatedUserImage(SQLiteDatabase db, int qbId) {

        String userImage = "";
        try {
            String query = "select * from " + DBTable.TBL_CHAT + " where " + DBTable.KEY_QB_FRIEND_ID + " = '" + qbId + "'";
            //  System.out.println("query : " + query);
            Log.d(TAG, "query friendImage from dataBase : - " + query);
            ...
            if (cursor.moveToFirst()) {
                ...
                do {
                    ...
                    String friendImage = cursor.getString(cursor.getColumnIndex(DBTable.KEY_FRIENDIMAGE));
                    userImage = friendImage;


                    //System.out.println("query : " + userImage);
                    Log.d(TAG, " friendImage from dataBase : - " + userImage);
        ...
        Log.d(TAG, " friendImage from dataBase 20 : - " + userImage);

        // closeConnecion();
        return userImage;
    }

    public static byte[] getChatDialogUsingQBId(SQLiteDatabase db, int qb_friend_id) {
        Log.v(TAG, "Inside Get Chat dialog details .... ");

        byte qbChatDialogBytes[] = null;

        try {
            String query = "select * from " + DBTable.TBL_QB_CHAT_DIALOG + " WHERE " + DBTable.KEY_QB_FRIEND_ID + " = '" + qb_friend_id + "'";
            Log.v(TAG, " getChatDialogUsingQBId from database : - " + query);

            Cursor cursor = db.rawQuery(query, null);
            if (cursor == null) {
                return qbChatDialogBytes;
            } else if (cursor.getCount() == 0) {
                return qbChatDialogBytes;
            }
            ...
            if (cursor.moveToFirst()) {
                do {
                    ...
                    int friend_qb_id = cursor.getInt(cursor.getColumnIndex(DBTable.KEY_QB_FRIEND_ID));
                    String dialog_id = cursor.getString(cursor.getColumnIndex(DBTable.KEY_QB_DIALOG_ID));
                    qbChatDialogBytes = cursor.getBlob(cursor.getColumnIndex(DBTable.KEY_QBCHATDIALOG_BYTES));
...
// Log.d(TAG, "Get full QBChatDialog 1 :- " + SerializationUtils.deserialize(qbChatDialogBytes));
            ...
            e.printStackTrace();
            System.out.println("query : recivername" + e.getMessage());
            Log.e(TAG, "Error in method getChatDialogUsingQBId() :- " + e.getMessage());
        }
        Log.v(TAG, " getChatDialogUsingQBId from database qbChatDialogBytes: - " + qbChatDialogBytes);

        return qbChatDialogBytes;
    }

    public static boolean ifChatDialogExists(SQLiteDatabase db, int qb_friend_id) {
        Log.v(TAG, "Inside Get Chat dialog details .... ");
...
//            cursor.close();
        ...
        Log.v(TAG, " getChatDialogUsingQBId from database qbChatDialogBytes: - " + exists);

        return exists;
    }

    public static byte[] getChatDialogUsingDialogID(SQLiteDatabase db, String dialog_id) {
        Log.v(TAG, "Inside Get Receiver details .... ");

        byte qbChatDialogBytes[] = null;

        try {
            String query = "select * from " + DBTable.TBL_QB_CHAT_DIALOG + " WHERE " + DBTable.KEY_QB_DIALOG_ID + " = '" + dialog_id + "'";
            Log.v(TAG, " getChatDialogUsingQBId from database : - " + query);
            ...
            if (cursor.moveToFirst()) {
                do {
                    ...
                    String type = cursor.getString(cursor.getColumnIndex(DBTable.KEY_QB_CHAT_TYPE));
                    qbChatDialogBytes = cursor.getBlob(cursor.getColumnIndex(DBTable.KEY_QBCHATDIALOG_BYTES));


                    Log.d(TAG, "Get chat type :- " + type);
                    Log.d(TAG, "Get dialog Details :- " + dialog_id);
                    Log.d(TAG, "Get qbChatDialogBytes 1 :- " + qbChatDialogBytes);
// Log.d(TAG, "Get full QBChatDialog 1 :- " + SerializationUtils.deserialize(qbChatDialogBytes));
