...
public class DataUpdate {
    ...
    public static void updateqbFileIdAndqbFileUid(SQLiteDatabase dbUpdate, int qbFileId, String qbFileUid, String dialogid, String messageid) {
// SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(DBTable.KEY_QB_FILE_UPLOAD_ID, qbFileId);
        contentValues.put(DBTable.KEY_QB_FILE_U_ID, qbFileUid);

        long idNew = dbUpdate.update(DBTable.TBL_CHAT, contentValues, DBTable.KEY_QB_DIALOG_ID + " = ? AND " + DBTable.KEY_QB_MESSAGE_ID + " = ? ",
                new String[]{dialogid, messageid});
        Log.d(TAG, "Update Read Status of message nyanya :- " + idNew);
        dbUpdate.close();
// dbUpdate.update("contacts", contentValues, "id = ? ", new String[] { Integer.toString(id) } );
    ...
    public static void UpdateFriendPro(SQLiteDatabase db, String image, String userStatus, String reciver) {
        ...
        db.update(DBTable.TBL_CHAT, initialValues, DBTable.KEY_QB_FRIEND_ID + " = ?",
                new String[]
                        {reciver});
...
//        initialValues.put(DBTable.KEY_MSGSTATUS, status);
        ...
        long id = db.update(DBTable.TBL_CHAT, initialValues, DBTable.KEY_QB_DIALOG_ID + " = ? " /*+ DBTable.KEY_QB_RECIVER_ID + " = ?"*/,
                new String[]{qbDialogId});
    ...
    public static void updateQBChatDialog(SQLiteDatabase db, int qb_id, String dialogId, byte[] data) {
        ...
        try {
            ...
            long id = db.update(DBTable.TBL_QB_CHAT_DIALOG, initialValues, DBTable.KEY_QB_FRIEND_ID + " = ? AND " + DBTable.KEY_QB_DIALOG_ID + " = ?",
                    new String[]{String.valueOf(qb_id), dialogId});
